...
async function findById(companyId, id) {
  const { rows } = await query(`SELECT * FROM departments WHERE id = $1 AND company_id = $2 AND is_deleted = FALSE`, [
    id,
    companyId,
  ]);
  return rows[0] || null;
...
async function create(companyId, { name }, createdBy) {
  const { rows } = await query(
    `INSERT INTO departments (company_id, name, created_by, updated_by)
     VALUES ($1, $2, $3, $3)
     RETURNING *`,
...
async function softDelete(companyId, id, deletedBy) {
  const { rows } = await query(
    `UPDATE departments SET is_deleted = TRUE, deleted_at = now(), deleted_by = $3
     WHERE id = $1 AND company_id = $2 AND is_deleted = FALSE
     RETURNING id`,
