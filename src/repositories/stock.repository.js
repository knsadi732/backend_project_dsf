 ...
 */
async function lockOrCreateForUpdate(client, companyId, warehouseId, productId) {
  await client.query(
    `INSERT INTO warehouse_stock (company_id, warehouse_id, product_id)
     VALUES ($1, $2, $3)
     ON CONFLICT (warehouse_id, product_id) WHERE is_deleted = FALSE DO NOTHING`,
    [companyId, warehouseId, productId],
  );

  const { rows } = await client.query(
    `SELECT * FROM warehouse_stock
     WHERE warehouse_id = $1 AND product_id = $2 AND is_deleted = FALSE
     FOR UPDATE`,
...
async function getStock(companyId, warehouseId, productId) {
  const { rows } = await query(
    `SELECT * FROM warehouse_stock
     WHERE company_id = $1 AND warehouse_id = $2 AND product_id = $3 AND is_deleted = FALSE`,
    [companyId, warehouseId, productId],
  );
  return rows[0] || null;
}
